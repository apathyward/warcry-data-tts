...
def serialize_ability_without_parent_key(abilities: Dict) -> str:
    abilities_lua = []
    for ability_id, ability_details in abilities.items():
        ability_lua = '{' + ', '.join([f'{k}="{v}"' if isinstance(v, str) else f'{k}={custom_serialize(v)}' for k, v in ability_details.items()]) + '}'
        abilities_lua.append(ability_lua)
    return '{' + ', '.join(abilities_lua) + '}'
...
def add_backslashes_to_inches(description: str) -> str:
    # Regular expression to find "number" followed by a double quote
    pattern = re.compile(r'(\d)"')
    # Replace found patterns with "number\" (adding a backslash before the double quote)
    return pattern.sub(r'\1\\"', description)
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Remove newlines from the entire abilities.lua content
        abilities_lua_content = remove_newlines(abilities_lua_content)
